...
export enum TerminalLowerCaseEnum {
  'PC' = 'pc',
  'MOBILE' = 'mobile',
  'ALL' = 'all',
}
export type PreviewInfo = {
  appId: string;
  appName: string;
  platform: TerminalLowerCaseEnum;
  rootDir: string;
};
export type Config = {
  appId: string;
  terminalType: TerminalLowerCaseEnum;
  deployId: string;
  extraData: {};
  isDependentApp: boolean;
  appName: string;
  sourceType: 'NORMAL' | 'FORM';
  pages?: string[] | undefined;
};

export const CACHE_DIR = `${PREVIEW_ROOT_DIR}/.esm`;
export const CACHE_DEP_DIR = path.join(CACHE_DIR, 'deps');
export const CACHE_DEP_METADATA_PATH = path.join(CACHE_DEP_DIR, '_metadata.json');
...
export const PREVIEW_CLIENT_SCRIPTS = path.join(process.cwd(), 'src/source-management/client');

export const SHARED_DEPS_PATH = path.join(process.cwd(), 'shared-deps');

export const SHARED_DEPS_PACKAGE_JSON_PATH = path.join(SHARED_DEPS_PATH, 'package.json');

export const SHARED_DEPS_NODE_MODULES_PATH = path.join(SHARED_DEPS_PATH, 'node_modules');
