    ...
    alias: Object.entries(ALIAS)
      .map<Alias>(([name, src]) => ({
        ...
        customResolver: (source: string, importer: string = '') => {
          ...
          const filePath = `/${appId}/${platform}${source.replace(root, '')}`;
      ...
      .concat([
        ...
        { find: /^[\/]app-styles-pc.css/, replacement: `${root}/app-styles-pc.css` },
        { find: /^[\/]app-styles-mobile.css/, replacement: `${root}/app-styles-mobile.css` },
        ...
        postcssUrl({
          url: (asset, dir) => {
            try {
              ...
              // no query here, try resolve appID and platform from path
              ...
              const resolved =
                appId &&
                platform &&
                tryResolveMemoizedFile(appId, platform, pureAppPath || 'invalid assets');
              if (resolved) {
